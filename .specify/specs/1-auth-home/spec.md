...
- Q: 密碼儲存的加密方式 → A: bcrypt 雜湊
- Q: Email 服務提供商選擇 → A: Resend Email API
- Q: 會話過期時間 → A: 30 天
- Q: 後端技術棧選擇 → A: Supabase Auth + Supabase Edge Functions (Hono Server)
- Q: 驗證碼錯誤嘗試次數限制後的行為 → A: 要求重新發送驗證碼
...
- **驗證碼輸入錯誤多次**: 使用者連續輸入錯誤驗證碼 5 次後，系統立即鎖定當前驗證碼並顯示錯誤訊息「驗證碼輸入錯誤次數過多，請重新發送驗證碼」，使用者必須透過「重新發送驗證碼」功能取得新驗證碼
- **Email 格式驗證**: 系統如何處理無效的 Email 格式（例如缺少 @、無效網域）？應在前端即時驗證並顯示錯誤訊息
- **會話過期**: 使用者長時間未使用 APP（超過 30 天），自動登入會話將過期，系統應要求重新登入
...
- **FR-054**: 系統必須過濾姓名欄位中的危險字元，僅允許中文字元（Unicode CJK）、英文字母（a-z, A-Z）、空格、連字號（-），禁止 SQL injection 字元（'、"、;、--）與 XSS 字元（<、>、&、script）
...
- **FR-020**: 系統必須追蹤每個驗證碼的錯誤嘗試次數
- **FR-021**: 系統必須在驗證碼錯誤嘗試達到 5 次時，立即鎖定該驗證碼並顯示錯誤訊息「驗證碼輸入錯誤次數過多，請重新發送驗證碼」

**登入功能**

- **FR-022**: 系統必須提供登入表單，包含 Email 和密碼欄位
- **FR-023**: 系統必須驗證使用者輸入的 Email 和密碼是否與資料庫中的帳號匹配
- **FR-024**: 系統必須在 Email 或密碼錯誤時，顯示統一錯誤訊息「Email 或密碼錯誤」（不透露具體是哪個欄位錯誤）
- **FR-025**: 系統必須在登入成功後，建立使用者會話（session）
- **FR-026**: 系統必須儲存會話資料到本地儲存（持久化），以支援自動登入功能
- **FR-027**: 系統必須在登入成功後，自動導向記帳主頁
- **FR-028**: 系統必須在 APP 啟動時，檢查是否存在有效會話
- **FR-029**: 系統必須在存在有效會話時，跳過登入頁面直接進入記帳主頁
- **FR-030**: 系統必須提供密碼欄位的「顯示/隱藏」切換按鈕
...
- **FR-031**: 系統必須在記帳主頁提供「登出」功能按鈕或選單項目
- **FR-032**: 系統必須在使用者點擊登出時，顯示確認對話框「確定要登出嗎？」
- **FR-033**: 系統必須在使用者確認登出後，清除所有本地儲存的會話資料
- **FR-034**: 系統必須在登出後，導向登入頁面

**記帳主頁功能**

- **FR-035**: 系統必須在使用者登入後顯示記帳主頁
- **FR-036**: 記帳主頁必須包含應用程式標題「記帳App」
- **FR-037**: 記帳主頁必須包含選單按鈕（漢堡選單圖示）
- **FR-038**: 記帳主頁必須包含通知按鈕（鈴鐺圖示）
- **FR-039**: 記帳主頁必須顯示「功能開發中」內容區域，包含：
  - 日曆圖示（圓形背景，15% 透明度綠色）
  - 標題「功能開發中」
  - 說明文字「記帳功能即將上線，敬請期待！」
- **FR-040**: 記帳主頁的選單按鈕必須包含「登出」選項

**一般性需求**

- **FR-041**: 系統必須在所有表單欄位提供即時驗證（回應時間 < 500ms），顯示錯誤訊息於欄位下方
- **FR-042**: 系統必須在處理使用者請求（註冊、登入、發送驗證碼）時，顯示載入指示器並禁用提交按鈕，防止重複提交
- **FR-043**: 系統必須在網路請求失敗時，顯示友善錯誤訊息「網路連線失敗，請檢查網路後重試」
- **FR-044**: 系統必須將所有 Email 地址統一轉換為小寫進行儲存和比對
- **FR-045**: 系統必須使用 Resend Email API 發送驗證碼郵件

**技術架構需求**

- **FR-046**: 後端系統必須使用 Supabase Auth 進行使用者認證管理
- **FR-047**: 後端 API 必須使用 Supabase Edge Functions 搭配 Hono Server 框架實作
- **FR-048**: 使用者密碼必須透過 Supabase Auth 的內建 bcrypt 雜湊機制儲存

### Non-Functional Requirements

**安全性**

- **NFR-001**: 系統必須實作 CORS 限制，僅允許已授權的前端網域存取 API（正式環境移除 * wildcard）
- **NFR-002**: 系統必須實作 Rate Limiting，每個 IP 每分鐘最多 60 次 API 請求
- **NFR-003**: 系統必須將驗證碼使用 bcrypt hash 儲存，錯誤嘗試次數達 5 次後立即鎖定

**錯誤處理**

- **NFR-004**: 系統必須統一錯誤回應格式，包含錯誤碼（error_code）、訊息（message）、詳細資訊（details）
- **NFR-005**: 系統必須在網路錯誤時顯示友善訊息並提供重試選項

**效能**

- **NFR-006**: 驗證碼發送到 Email 的時間在 30 秒內（95% 的情況下），使用 Supabase Edge Functions 日誌記錄測量 P95 延遲
- **NFR-007**: 系統能夠處理至少 100 個並發註冊請求而不影響回應時間（每個請求在 3 秒內完成），使用 k6 或 Artillery 進行負載測試驗證

**可維護性**

- **NFR-008**: 所有 API 端點必須有單元測試覆蓋，覆蓋率 ≥ 80%
- **NFR-009**: 關鍵使用者流程（註冊→驗證→登入→登出）必須有整合測試覆蓋

### Key Entities

- **使用者 (User)**: 代表註冊的應用程式使用者
  - 屬性：使用者 ID（唯一識別碼）、姓名、Email（唯一）、密碼（使用 bcrypt 雜湊儲存）、帳號狀態（已驗證/未驗證）、註冊時間、最後登入時間
  - 關聯：一個使用者可以有多個會話記錄

- **驗證碼 (VerificationCode)**: 代表發送給使用者進行 Email 驗證的臨時驗證碼
  - 屬性：驗證碼 ID、6 位數字碼、關聯的 Email、建立時間、過期時間（建立時間 + 5 分鐘）、錯誤嘗試次數、使用狀態（未使用/已使用/已過期/已鎖定）
  - 關聯：每個驗證碼關聯到一個註冊請求（Email）

- **會話 (Session)**: 代表使用者的登入會話
  - 屬性：會話 ID、使用者 ID、建立時間、最後活動時間、過期時間（建立時間 + 30 天）、裝置資訊（可選）、會話狀態（有效/已登出/已過期）
  - 關聯：每個會話屬於一個使用者
